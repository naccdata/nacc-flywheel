...
class APOETransformerVisitor(GearExecutionEnvironment):
    ...
    def create(
        ...
        if not target_project_id:
            target_project_id = file_input.parents.project
    ...
    def run(self, context: GearToolkitContext) -> None:
        ...
        target_project = self.proxy.get_project_by_id(self.__target_project_id)
        if not target_project:
            raise GearExecutionError(
                f'Did not find a project with ID {self.__target_project_id}')

        log.info(type(target_project))
