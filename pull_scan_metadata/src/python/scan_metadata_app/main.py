...
def split_table(table_data, table_name):
    """
    Splits the table data into center subsets and uploads them to a FW project
    """
    ...
    if 'ADCID' in table_data.columns:
        ...
        for adcid in unique_adcid:
            ...
            center_subset = table_data[table_data['SITE'].str.contains(f'\(ADC\s?{adcid}\)')]
        ...
        log.error(f'{table_name} does not have a recognized column for the Center ID')

def get_s3_client():
    # Get S3 credentials
    access_key = get_parameter(parameter_name='/prod/flywheel/gearbot/loni/accesskey')
    secret_key = get_parameter(parameter_name='/prod/flywheel/gearbot/loni/secretkey')
    region = 'us-west-2'
...
def run():
    """Pulls SCAN metadata from S3, splits the data by center, and uploads the data to the center-specific FW project.

    Args:
      flywheel_proxy: the proxy for the Flywheel instance
    """
    table_list = ["v_scan_upload_with_qc", "v_scan_mri_dashboard", "v_scan_pet_dashboard"]
    
    s3_client = get_s3_client()
    try:
        for table_name in table_list:
            log.info(f"Downloading {table_name} from S3")
            table_data = download_table(s3_client, file_name=f"{table_name}.csv")
